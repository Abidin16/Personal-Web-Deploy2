...
app.use("/uploads", express.static(__dirname + "/uploads")); //   set folder to uploads
  ...
  db.connect(function (err, client, done) {
    ...
    client.query(query, function (err, result) {
      ...
      data = data.map(function (blog) {
        //manipulasi array  membuat edit dan delete
        return {
          ...blog,
  ...
  db.connect(function (err, client, done) {
    ...
    client.query(query, function (err, result) {
      ...
      if (result.rows.length == 0) {
        //mengatasi klo ngga ada password & email
      ...
      if (isMatch) {
        //session penyimpanan sementara di sisi client
